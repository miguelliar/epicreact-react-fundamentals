...
Whenever we create a component we can validate the properties we are passing it by using ```propTypes``` as the following:
```js
Component.propTypes = {
    property(props, propName, componentName) {
        const type = typeof props[propName]
        if (type !== 'string') {//Or any other type checking
            new Error("Some message here")
        }
    }
}
```

This is useful and can be even more improved. If we extract the behaviour required for handling errors/types that are 
the same in different components you could do:
```js
const PropTypes = {
...
Component.propTypes = {
    property: PropTypes.type, //By doing this we reuse code and is more optimal
    secondProperty: PropTypes.type
}
```
Anyways, typescript is much more useful and used

What is really useful are React Fragments. React Fragments is a component that allows returning two
elements side by side and not nested.
For example:
```jsx
const Nested = (
    <div>
...
React.createElement('div',
    React.createElement('td', "First element"), 
    React.createElement('td', "First element")
)
```
```jsx
...
This can be pretty useful in cases where several elements are needed to be returned not enveloped in a container.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=uo270927%40uniovi.es).
